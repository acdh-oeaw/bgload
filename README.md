...
6. Get the fresh clone of your database from varuna. The database name ends with .jnl and can be provided by ACDH sysadmins. The database should be copied to the bgload dir.
...
To copy and deploy new databasae to varuna make sure that there is enough free space on the disk on varuna.
It is NAS partition that is mounted in /opt/jetty/acdh_base
